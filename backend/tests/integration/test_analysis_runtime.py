...
def test_analysis_python_session_persists_state_and_uses_runtime_control_plane(
    tmp_path: Path,
) -> None:
...
def test_analysis_executor_preserves_success_contract(tmp_path: Path) -> None:
    session = LocalDockerPythonSession(
        timeout_seconds=30,
    ...
    try:
        with executor.analysis_python_session_scope(session):
            container_id = session._backend.container_id
