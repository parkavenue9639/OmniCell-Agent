...
不实际调用任何外部 LLM：通过 monkeypatch 将角色 alias facade 替换为
...
def _patch_llm(monkeypatch, profile: ContextProfile) -> None:
    monkeypatch.setattr(
        cr.llm,
        "get_llm_by_alias",
        lambda *args, **kwargs: _FakeLLM(profile),
    )
...
def test_sandbox_path_mapping_rejects_conversation_workspace_escape(tmp_path):
    with pytest.raises(ValueError, match="逃逸"):
        cr._sandbox_path_to_host(
            "/app/data/../../outside.h5ad",
            str(tmp_path),
        )
