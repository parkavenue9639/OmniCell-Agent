...
from omnicell_agent.annotation import graph as annotation_engine
from omnicell_agent.annotation.nodes import annotator, validator
...
def test_marker_contract_normalizes_analysis_export_fields(tmp_path: Path) -> None:
...
def test_annotation_distribution_contract(tmp_path) -> None:
    contract_path = _write_contract(tmp_path)

    sends = annotation_engine.distribute_clusters(
        {
    ...
    projection = [
        {
            ...
            "top_marker_evidence": send.arg["top_marker_evidence"],
    ...
    assert projection == [
        {
            ...
            "top_marker_evidence": [
                (
                    f"T_MARKER_{index}: log2FC={2.0 - (index * 0.1):.3g}, "
                    f"pct_in=0.8, pct_out=0.1, delta_pct=0.7, "
                    f"p_adj={0.01 * (index + 1):.3g}"
                )
        ...
        },
        {
            ...
            "top_marker_evidence": [
                (
                    f"B_MARKER_{index}: log2FC={2.0 - (index * 0.1):.3g}, "
                    f"pct_in=0.8, pct_out=0.1, delta_pct=0.7, "
                    f"p_adj={0.01 * (index + 1):.3g}"
                )
                for index in range(5)
...
def test_annotation_post_scorer_route_contract(
    ...
    monkeypatch.setattr(annotation_engine, "ENABLE_BOOST", boost_enabled)
    state = {"quality_scores": {"cs_score": score}, "retry_count": retry_count}

    assert annotation_engine.post_scorer_route(state) == expected
...
def test_annotation_score_contract(
    sub_type: str,
    marker_count: int,
    penalty: int,
    self_ok: float,
    ...
    result = scorer_node(
        {
            ...
            "top_marker_evidence": [
                (
                    f"G{i}: log2FC=2, pct_in=0.8, pct_out=0.1, "
...
def test_annotation_score_fails_closed_without_quantitative_marker_evidence() -> None:
    result = scorer_node(
        {
            "cluster_id": "0",
            "top_n_markers": ["IL7R", "CCR7", "LTB", "MALAT1", "CD3D"],
            "predictions": {"sub_type": "CD4 T cells"},
            "quality_scores": {
                "validator_penalty": 0,
    ...
    assert result["quality_scores"]["cs_score"] == 0.0


def test_annotation_projection_preserves_quantitative_differences(
    tmp_path: Path,
) -> None:
    path = tmp_path / "quantitative-markers.json"
    MarkerTableContract(
        markers=[
            MarkerGene(
                gene_name="SHARED",
    ...
    assert sends[0].arg["top_n_markers"] == sends[1].arg["top_n_markers"]
    assert (
        sends[0].arg["top_marker_evidence"]
        != sends[1].arg["top_marker_evidence"]
    )
    assert "log2FC=3" in sends[0].arg["top_marker_evidence"][0]
    assert "delta_pct=0.05" in sends[1].arg["top_marker_evidence"][0]


def test_initial_annotator_receives_quantitative_marker_evidence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    class StructuredModel:
        def invoke(self, messages):
            ...
            return annotator.AnnotationOutput(
                reasoning_chain="Bounded evidence assessment",
                general_type="Immune cells",
                sub_type="T cells",
                marker_evidence=["IL7R has cluster-enriched expression"],
            )
    ...
    monkeypatch.setattr(annotator, "ENABLE_SELF_CONSISTENCY", False)
    monkeypatch.setattr(
        annotator.llm,
        "get_llm_by_alias",
        lambda *args, **kwargs: ControlledModel(),
    )

    result = annotator.annotator_node(
        {
            "cluster_id": "0",
            "species": "Human",
            "tissue": "PBMC",
            ...
            "top_marker_evidence": [
                (
                    "IL7R: log2FC=3, pct_in=0.9, pct_out=0.1, "
                    "delta_pct=0.8, p_adj=0.001"
                )
            ],
            "predictions": {},
            "quality_scores": {},
    ...
    assert result["predictions"]["sub_type"] == "T cells"
    assert "log2FC=3" in str(captured_messages[1].content)
    assert "pct_out=0.1" in str(captured_messages[1].content)


def test_validator_receives_same_quantitative_marker_evidence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured_messages: list[Any] = []
    ...
    class StructuredModel:
        def invoke(self, messages):
            ...
            return validator.ValidatorOutput(
                ...
                critique="Quantitative panel supports a provisional broad label.",
            )
    ...
    result = validator.validator_node(
        {
            "cluster_id": "0",
            "species": "Human",
            "tissue": "PBMC",
            ...
            "predictions": {
                "sub_type": "T cells",
                "reasoning_chain": "Bounded evidence assessment",
                "marker_evidence": ["IL7R supports the provisional label"],
            },
...
def test_annotation_result_projection_contract(
    ...
    monkeypatch.setattr(
        annotation_engine,
        "single_cluster_app",
        ControlledClusterApp(),
    )

    assert annotation_engine.process_cluster_wrapper({"cluster_id": "7"}) == {
        "cluster_annotations": {
            "7": {
                "general_type": "Immune cells",
                "sub_type": "Rare cells",
...
def test_annotation_controlled_end_to_end_contract(
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(annotation_engine, "ENABLE_BOOST", True)
    monkeypatch.setattr(
        annotation_engine,
        "ENABLE_CONSISTENCY_REVIEWER",
        True,
    )
    contract_path = _write_contract(tmp_path)

    final_state = annotation_engine.build_annotation_graph().invoke(
        {
            "contract_file_path": contract_path,
            "species": "Human",
            "tissue": "PBMC",
    ...
    assert final_state["final_report"] == "\n".join(
        [
            ...
            "| :---: | :--- | :--- | :---: | :--- | :--- |",
            "| 0 | Immune cells | **CD4 T cells** | 95.0 | — | No automatic review flag |",
            "| 1 | Immune cells | **B cells** | 95.0 | — | No automatic review flag |",
            "\n## 需人工复核清单 (Manual review queue)\n",
            "_没有 cluster 被自动规则标记为需要人工复核。_",
        ]
    )
