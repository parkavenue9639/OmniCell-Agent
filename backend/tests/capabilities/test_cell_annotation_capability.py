...
def test_inspection_sorts_markers_and_reports_bounded_truncation(tmp_path: Path) -> None:
    ...
    result = InspectMarkerTableCapability().invoke(
        InspectMarkerTableRequest(
...
def test_capabilities_reject_foreign_conversation_artifact(tmp_path: Path) -> None:
    ...
    with pytest.raises(ArtifactBoundaryError, match="当前 conversation"):
        InspectMarkerTableCapability().invoke(
            InspectMarkerTableRequest(marker_table=foreign_ref),
            context,
        )
    with pytest.raises(ArtifactBoundaryError, match="当前 conversation"):
...
def test_workflow_projects_input_and_publishes_full_outputs(tmp_path: Path) -> None:
    ...
    capability = CellAnnotationCapability(graph_factory=RecordingGraph)
    ...
    assert "internal/cell-annotation" in pinned_contract.as_posix()
    ...
    assert [
        ...
        {
            "cluster_id": cluster_id,
            "general_type": annotation["general_type"],
...
def test_annotation_capability_preserves_review_projection(
    tmp_path: Path,
    ...
    graph_projection = annotation_engine.process_cluster_wrapper(
        {"cluster_id": "7"}
    )
    ...
    result = CellAnnotationCapability(graph_factory=ProjectionGraph).invoke(
        CellAnnotationRequest(
            marker_table=marker_ref,
            species="Human",
            tissue="PBMC",
...
def test_workflow_rejects_incomplete_annotation_terminal_projection(
    tmp_path: Path,
    final_state: dict,
) -> None:
    context, store = _artifact_context(tmp_path / "conversation")
    ...
    with pytest.raises(CapabilityExecutionError, match="未完整收敛|有效"):
        CellAnnotationCapability(graph_factory=IncompleteGraph).invoke(
            CellAnnotationRequest(
                marker_table=marker_ref,
                species="Human",
                tissue="PBMC",
