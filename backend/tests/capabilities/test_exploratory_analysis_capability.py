...
def test_context_tool_projects_typed_context_without_exposing_node_state(
    ...
    result = InspectDatasetCapability(resolver).invoke(
...
def test_exploratory_analysis_owns_scope_and_returns_artifact_projection(
    tmp_path: Path,
) -> None:
    context, dataset = _context(tmp_path)
    ...
    assert graph.initial_state["marker_table_path"].startswith(
        "/app/data/artifacts/exploratory-analysis/"
    )


def test_exploratory_analysis_projects_max_retry_as_aborted(tmp_path: Path) -> None:
    context, dataset = _context(tmp_path)
    capability = ExploratoryAnalysisCapability(
...
def test_exploratory_analysis_allows_completed_non_marker_goal_without_marker(
    tmp_path: Path,
) -> None:
    context, dataset = _context(tmp_path)
    capability = ExploratoryAnalysisCapability(
...
def test_exploratory_analysis_rejects_marker_symlink_before_contract_parser(
    tmp_path: Path,
...
def test_exploratory_analysis_rejects_marker_replacement_before_contract_parser(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    context, dataset = _invocation_context(tmp_path)
...
def test_exploratory_analysis_rejects_non_terminal_projection(tmp_path: Path) -> None:
    context, dataset = _context(tmp_path)
    capability = ExploratoryAnalysisCapability(
...
def test_exploratory_analysis_preserves_controlled_engine_contract(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    context, dataset = _context(tmp_path)
    ...
    capability = ExploratoryAnalysisCapability(
        scope_factory=lambda _workspace: executor.analysis_python_session_scope(
    ...
    assert [
        ...
        ("deterministic", "导出 marker contract", "completed"),
    ]
    assert "recipe_name" not in result.model_dump_json()
