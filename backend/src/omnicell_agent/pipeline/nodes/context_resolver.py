...
def _project_root() -> str:
    # context_resolver.py -> nodes -> pipeline -> omnicell_agent -> src -> backend -> <repo root>
    return os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", ".."))


def _sandbox_path_to_host(
    sandbox_path: str,
    ...
    if sandbox_path.startswith("/app/data/"):
        ...
        root = Path(
            conversation_workspace or os.path.join(_project_root(), "data")
        ).resolve()
        host_guess = (root / rel).resolve(strict=False)
        try:
            host_guess.relative_to(root)
        except ValueError as exc:
            raise ValueError("sandbox data path 逃逸 conversation workspace") from exc
        if host_guess.exists():
            return str(host_guess)
...
def _structured_extract(user_prompt: str, h5ad_meta: Dict[str, Any], filename_hints: Dict[str, str]) -> ContextProfile:
    ...
    try:
        model = llm.get_llm_by_alias(llm.LLMRole.FAST_ROUTER, temperature=0.0)
        structured_llm = model.with_structured_output(ContextProfile)
...
def run_context_resolver(state: DataPipeline_State) -> Dict[str, Any]:
    ...
    conversation_workspace = str(
        (state.get("task_context", {}) or {}).get("conversation_workspace", "")
    ).strip()
    host_path = _sandbox_path_to_host(raw_data_path, conversation_workspace or None)
